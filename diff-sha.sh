...
if [[ -z $GITHUB_BASE_REF ]]; then
  ...
  if [[ -z $INPUT_BASE_SHA ]]; then
    git fetch --no-tags -u --progress --depth=30 origin "$TARGET_BRANCH":"$TARGET_BRANCH"
    PREVIOUS_SHA=$GITHUB_EVENT_PULL_REQUEST_BASE_SHA && exit_status=$? || exit_status=$?
    echo "::debug::Previous SHA: $PREVIOUS_SHA"
  else
    git fetch --no-tags -u --progress --depth=30 origin "$INPUT_BASE_SHA" && exit_status=$? || exit_status=$?
